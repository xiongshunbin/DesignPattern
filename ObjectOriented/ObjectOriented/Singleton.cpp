...
/**	饿汉模式的缺点:
*		1.启动时间变长: 如果有很多单例, 都在程序启动时初始化, 会拖慢启动速度.
*		2.静态初始化顺序问题: 如果单例A依赖单例B, 而A在B之前初始化, 就会出问题, 因为C++不保证不同编译单元的静态变量初始化顺序.
*		3.资源浪费: 即使单例未使用,也会占用资源.
*/
...
// C++11标准保证局部静态变量初始化的线程安全性, 编译器会自动处理同步问题.
class TaskQueue
{
public:
	TaskQueue(const TaskQueue& t) = delete;
